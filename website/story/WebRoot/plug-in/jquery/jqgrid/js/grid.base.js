...
var j_jqgrid_rowlist;
var j_jqgrid_rownum;
/*
$.ajax({
	url:'mainServlet.html?jqgridparams=getparams',
	timeout:500,
	async:false,
	type:'post',
	success:function(data){
		$(data).find("row").each(function(){
			j_jqgrid_rowlist = $(this).attr("jqGridRowList");
			j_jqgrid_rownum = $(this).attr("jqGridRowNum");
		});
	}
});
*/
/**
 * liuyan
 * 默认的jqgrid的初始默认每页显示记录数
 * @param {Object} p
 * @memberOf {TypeName} 
 * @return {TypeName} 
 */
if(j_jqgrid_rownum==undefined) j_jqgrid_rownum = 15;
if(j_jqgrid_rowlist==undefined)
	j_jqgrid_rowlist = [10,15,20];
else
	j_jqgrid_rowlist = eval("(" + j_jqgrid_rowlist + ")");
...
$.fn.jqGrid = function( p ) {
	...
	multiboxonly : false,
	scrollrows : false
	}, $.jgrid.defaults, p || {},p.useDefault==true?{}:{rowList:p.rowList==undefined?[]:j_jqgrid_rowlist,rowNum:j_jqgrid_rownum});
	var grid={         
		headers:[],
		cols:[],
		...
		var updatepager = function() {
			if(ts.p.pager) {
				...
				if (ts.p.viewrecords){
					...
					$('#sp_2',ts.p.pager).html(ts.p.myZN+"&#160;"+ts.p.records+"&#160;"+ts.p.recordtext+"&#160;");
				}
