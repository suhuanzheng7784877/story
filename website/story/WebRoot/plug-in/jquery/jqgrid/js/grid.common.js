...
function createModal(aIDs, content, p, insertSelector, posSelector, appendsel) {
	...
	var mh = jQuery('<div id="'+aIDs.modalhead+'"  style="float:left;"><table width="100%" style="float:left;"><tbody style="float:left;"><tr><td class="modaltext">'+p.caption+'</td> <td style="text-align:right;float:left;" ><a href="javascript:void(0);" class="jqmClose">'+(clicon!=''?'<img src="' + clicon + '" border="0"/>':'X') + '</a></td></tr></tbody></table> </div>').addClass("modalhead");
	...
	jQuery(loading).css("float","left");
	jQuery(mw).prepend(loading);
	jQuery(mw).prepend(mh);
	jQuery(mw).addClass("jqmWindow");
	jQuery(mc).css("float","left");
	jQuery($(aIDs)).css("float","left");
	jQuery(mw).css("float","left");
	jQuery(mw).find("table").css("float","left");
	if (p.drag) {
