...
//              2011-1-5  add select true value
//
...
if(jQuery) (function($){
	...
	$.extend($.fn, {
		multiSelect: function(o, callback,id,symbol) {
			// Default options
			if( !o ) var o = {};
			if( o.selectAll == undefined ) o.selectAll = true;
			if( o.selectAllText == undefined ) o.selectAllText = "全部选中";
			...
			$(this).each( function() {
				var select = $(this);
				var html = '<input type="text" id="'+id+'" readonly="readonly" class="multiSelect" value="" style="cursor: default;" />';				
				...
				html += '</div>';
				$(select).after(html);
				$(".multiSelectOptions",$(select).parent()).css("width",$(select).css("width"));
				...
				$(select).next('.multiSelect').next('.multiSelectOptions').find('INPUT:checkbox').click( function() {
					$(this).parent().parent().multiSelectUpdateSelected(o,symbol);
				...
				$(select).next('.multiSelect').next('.multiSelectOptions').each( function() {
					$(this).multiSelectUpdateSelected(o,symbol);
					...
					if( $(this).next('.multiSelectOptions').is(':visible') ) {
						...
						if( e.keyCode == 13 || e.keyCode == 32 ) {
							...
							if( $(this).next('.multiSelectOptions').find('LABEL.hover INPUT:checkbox').hasClass('selectAll') ) {
								...
								$(this).next('.multiSelectOptions').multiSelectUpdateSelected(o,symbol);
								if( callback ) callback($(this));
								return false;
							}
							// Other checkboxes
		...
		multiSelectOptionsShow: function() {
			...
			$(this).next('.multiSelectOptions').css({ left: (offset.left-4) + 'px' });
		...
		multiSelectUpdateSelected: function(o,symbol) {
			var i = 0, s = '';
			$(this).find('INPUT:checkbox:checked').not('.selectAll').each( function() {
				i++;
			})
			if( i == 0 ) {
				$(this).prev('INPUT.multiSelect').val( o.noneSelected ).attr("trueval","");
			} else {
				if( o.oneOrMoreSelected == '*' ) {
					...
					$(this).find('INPUT:checkbox:checked').each( function() {
						if( $(this).parent().text() != o.selectAllText ){
							display = display + $(this).parent().text() + symbol+'';
							display_val = display_val + $(this).val() + symbol+'';
						}
					});
					display = display.substr(0, display.length - 1);
					display_val = display_val.substr(0, display_val.length - 1);
					$(this).prev('INPUT.multiSelect').val( display );
					$(this).prev('INPUT.multiSelect').attr("trueval", display_val );
				} else {
