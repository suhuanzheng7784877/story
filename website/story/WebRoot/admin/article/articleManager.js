...
function openGridJson() {
	...
	jQuery("#editgrid").jqGrid(
			{
				url : "../../article/showAllArticles.action",
				datatype : "json",
				colNames : [ '操作', '主键', '异文标题', '作者', '是否通过审核', '上线日期' ],
				colModel : [ {
					name : 'viewOperation',
					index : 'viewOperation',
					sortable : false,
...
function loadDataByIdKey(id) {
	$.ajax( {
		url : '../../article/showOneArticles.action?articleId=' + id,
		datatype : 'json',
		...
		var isPass = jsonstrObj.result.isPass;
...
function deleteOneObject(key) {
	if (confirm("您确定要删除该记录吗？")) {
		$.ajax( {
			url : '../../article/delete.action?id=' + key,
			datatype : 'json',
			cache : false,//如果值变化可能性比较大 一定要将缓存设成false
			timeout : 1000,
			async : false,//同步方式
...
function serchObjectSubmit() {
	...
	var requestUrl = basePath + "/article/searchArticles.action?" + con;
	//更改url重新加载
