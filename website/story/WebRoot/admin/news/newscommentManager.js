...
function openGridJson() {
	...
	jQuery("#editgrid").jqGrid(
			{
				url : "../../news/showNewsComment.action",
				datatype : "json",
				colNames : [ '操作', '主键', '新闻标题', '评论人','评论日期', '评论内容' ],
				colModel : [ {
					name : 'viewOperation',
					index : 'viewOperation',
					sortable : false,
...
function deleteOneObject(key) {
	if (confirm("您确定要删除该记录吗？")) {
		$.ajax( {
			url : '../../news/deleteComment.action?id=' + key,
			datatype : 'json',
...
function serchObjectSubmit() {
	...
	var requestUrl = basePath + "/news/searchNewsComment.action?" + con;
